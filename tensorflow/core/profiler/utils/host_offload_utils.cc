...
void HostOffloadEventProcessor::ProcessHostOffloadOpEvent(
    ...
    XPlaneBuilder& plane_builder, XLineBuilder& host_offload_op_line_builder) {
  ...
  if (absl::StrContains(event.DisplayName(), "-start")) {
    ...
    XEventMetadata* host_offload_copy_metadata =
        plane_builder.CreateEventMetadata();
    host_offload_copy_metadata->set_display_name(display_opname);
    XEventBuilder event_builder =
        host_offload_op_line_builder.AddEvent(*host_offload_copy_metadata);
    ...
    const XStatMetadata& async_stat = *plane_builder.GetOrCreateStatMetadata(
