...
class LockFreeQueue {
 ...
 public:
  ...
  LockFreeQueue& GrabWhenProducerStopped(LockFreeQueue& src) {
    this->Clear();
  ...
  LockFreeQueue& GrabWhenProducerActive(LockFreeQueue& src) {
    this->Clear();
    auto* empty_block = this->start_block_;
    this->start_block_ = this->end_block_ = nullptr;
    this->start_ = src.start_;
    // Use the end we see now, skip further growing in src thread if any
    size_t end = src.End();
    this->SetEnd(end);
    while (src.start_block_->start + Block::kNumSlots <= end) {
      auto* old_block = std::exchange(src.start_block_, src.start_block_->next);
      src.start_ = src.start_block_->start;
    ...
    empty_block->start = src.start_block_->start;
    ...
    size_t bs = src.start_block_->start;
    for (size_t i = std::max(src.start_, bs); i < end; i++) {
      auto& src_slot = src.start_block_->slots[i - bs];
      auto& dst_slot = this->end_block_->slots[i - bs];
