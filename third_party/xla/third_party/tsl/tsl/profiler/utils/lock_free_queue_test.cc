...
void TestGrabFrom(size_t event_count1, size_t event_count2, Generator gen) {
  ...
  if constexpr (producer_concurrent) {
    dumped_queue1.GrabWhenProducerActive(queue);
  } else {
    dumped_queue1.GrabWhenProducerStopped(queue);
  }
  std::vector<T> result1;
  ...
  if constexpr (producer_concurrent) {
    dumped_queue2.GrabWhenProducerActive(queue);
  } else {
    dumped_queue2.GrabWhenProducerStopped(queue);
  }
  std::vector<T> result2;
