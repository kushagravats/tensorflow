...
T EvaluatePolynomial(T x, const std::array<T, N>& coeffs) {
  // Evaluate the polynomial as accurately as we can using
  // double precision and FMA.
  double result = 0;
...
UNARY_TEST_FLOAT_32_BITS_OR_LESS(Cbrt, {
  ...
  // For CPU we allow a small absolute error (e.g. ~9e-16 for F32), since
  // Cbrt is not accurate for subnormal inputs. This corresponds to a 0.5%
  // relative error for the smallest normalized floating point values.
  //
  // In contrast, GPUs and TPUs flush subnormal inputs to zero, which the test
  // harness counts as accurate.
  if (IsCpu(platform_)) {
    error_spec_gen = +[](NativeT x) {
