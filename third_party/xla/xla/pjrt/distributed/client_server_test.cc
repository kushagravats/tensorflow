...
TEST_F(ClientServerTest, KeyValueSet_Duplicate_Fails) {
  ...
  auto client = GetClient(/*node_id=*/0);
  TF_ASSERT_OK(client->Connect());
  TF_ASSERT_OK(client->KeyValueSet("duplicate_key", "value1"));
  EXPECT_TRUE(
      absl::IsAlreadyExists(client->KeyValueSet("duplicate_key", "value2")));
}
...
TEST_F(ClientServerTest, KeyValueSet_Duplicate_Overwrites) {
  ...
  TF_ASSERT_OK(client->KeyValueSet("duplicate_key", "value1"));
  TF_EXPECT_OK(
      client->KeyValueSet("duplicate_key", "value2", /*allow_overwrite=*/true));
}
