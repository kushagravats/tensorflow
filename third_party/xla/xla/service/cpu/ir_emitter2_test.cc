...
static absl::StatusOr<HloInstruction*> FindInstruction(HloModule& module,
...
TEST(IrEmitter2Test, BuildKernelPrototype) {
...
TEST(IrEmitter2Test, EmitElementalKernel) {
  llvm::LLVMContext context;
  auto module = std::make_unique<llvm::Module>("test", context);
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto* convert, FindInstruction(*hlo, "convert"));
